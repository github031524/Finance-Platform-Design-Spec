...
| Event dates — a report date, an expiry, a next/prev date, any date column | weekday first, comma, then the date: `Wed, Sep 9`; add the year only when it isn't this year: `Wed, Sep 10, 2025`. Three-letter weekday, three-letter month | `Sep 9` (weekday dropped), `Wednesday, September 9`, `Sept 9`, `09/09/2025`, `2025-09-09` |
| Timestamps — when the app fetched, refreshed or computed something | `Jul 19`, no weekday; add the year only when it isn't this year: `Jul 19, 2025` | `Sat, Jul 19` in a status line, `07/19/2025`, `2025-07-19` |
...
**Two kinds of date.** An *event* date says when something happens — a report date, an expiry, the next/prev date in an earnings list, any date column in a table — and **carries the weekday**, because the day of the week is part of the information: an earnings week is read by day, and "before the open Wednesday" is how the timing is used. A *timestamp* says when the app last did something — fetched, refreshed, computed — and doesn't. They are two different figures with two different rules, like a price and a percentage: never strip the weekday from an event date to make it "match" a timestamp, and never add one to a timestamp. Sorting stays chronological whatever the display (Sortable columns, below).
...
**Column-header tooltips** — every table column header carries a plain-language definition via the native HTML `title` attribute. **No custom tooltip component, no JavaScript, no CSS** — on hover (~1s browser delay) the browser renders its default tooltip: system font and size, positioned at the cursor. Styling is deliberately left to the browser/OS. **The text is one sentence in plain trader's English — standard market terms are fine, textbook definitions are not — and as short as it can be: never more than 20 words, and usually far fewer.** Say what the figure is; spell out how it is worked out only when the header doesn't already make that obvious. A tooltip is read at a glance or skimmed past:

```html
<th class="num" title="Total revenue for the trailing twelve months">REVENUE (TTM)</th>
```
...
It downloads the current `styles.css`, `fonts/`, `logo.svg` and `favicon.png` from this repo's `main` branch, and puts the shared **`CLAUDE.md`** at the app's repo root — the house rules for how Claude works in every app (no changes without a go-ahead, auto-PR and auto-merge, stay on the one job, plain-English replies with no preambles or recaps, one action per step) — so "bring the app up to the spec" starts with one command instead of five copy-pastes; copying by hand is exactly how apps fall behind. `CLAUDE.md` is identical in every app, like `styles.css`: the script replaces whatever was there, so app-specific notes for Claude don't belong in it. It refreshes files, not code: markup the spec asks for (the company cell, the toolbar, the switcher, sort headers) still needs reading the spec.
...
7. Delete descriptions and sell copy; labels become short uppercase. Any status/freshness text moves inline into the relevant toolbar row (no context bar to put it in). Spell every number and date per §06 Numbers & dates — signed deltas with a real minus sign, one-decimal percentages, `Wed, Sep 9` event dates (weekday kept), `Jul 19` timestamps.
...
13. Give every column header a native `title` tooltip — one short sentence in plain trader's English, never more than 20 words (§06).
...
Open a Claude Code session **inside the app's own repo** — the spec is fetched from this repo, never worked in, and a session opened here has no app to change — and paste:

> This is the **Stock Screener**. Fetch the latest `main` of `github031524/Finance-Platform-Design-Spec`, read the whole spec, and run its pick-your-changes mode (§09a).

That is the whole prompt. Swap the name for another app — a §04b registry name, verbatim. Naming the app removes the one question the session would otherwise stop for; naming the repo makes it read the spec fresh rather than from memory or an older copy. Everything else — the inventory, the rounds, the shared-file refresh, the checks, the PR, the report — is the protocol's job, so it isn't restated in the prompt: a long prompt only drifts from the spec it paraphrases.

### Session rules

This mode stops and waits for the user several times — once per menu round, and once more for anything that needs asking — and every stop is a place a run has come off the rails before. For the whole session:

- **One agent, in order.** No subagents, no orchestration, no multi-agent "workflow" skills: this is one repo and one sequence of steps, and each round's answers must land in the conversation that asked for them.
- **Waiting means waiting.** While a question is open, do nothing else — no background scan, no parallel work. Whatever is still running when the answer arrives is killed with the turn, and the session resumes having lost its place.
